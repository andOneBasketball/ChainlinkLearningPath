...
contract VRFTask is VRFConsumerBaseV2 {
    ...
    uint32 constant CALL_BACK_LIMIT = 300000;
     ...
     * 关于如何使得获取的随机数不重复，清参考以下代码 (Fisher-Yates Shuffle)
        ...
        override
    {
        ...
        uint256 last_item = NUM_WORDS - 1;
        for (uint256 i = 1; i < NUM_WORDS - 1; i++) {
            uint256 selected_item = _randomWords[i] % last_item;
            (s_randomWords[last_item], s_randomWords[selected_item]) = (s_randomWords[selected_item], s_randomWords[last_item]);
            last_item--;
        }
